...
export default function GuardrailsScreen() {
  ...
  const storedGuardrails = useKidsProfileStore(state => state.guardrails);

  const [selected, setSelected] = useState(storedGuardrails ?? []);

  useEffect(() => {
    if (storedGuardrails?.length) {
      setSelected(storedGuardrails);
    }
  }, [storedGuardrails]);
  ...
  const setGuardrails = useKidsProfileStore(state => state.setGuardrails);
